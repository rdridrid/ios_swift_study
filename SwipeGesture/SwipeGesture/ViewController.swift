...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        imgUp.append(UIImage(named: "arrow-up-black.png")!)//인덱스 0부터 시작
        ...
        swipeUp.direction = UISwipeGestureRecognizer.Direction.up //action인수는 해당 스와이프 제스처 시 실행될 메서드
        self.view.addGestureRecognizer(swipeUp)
        
        let swipeDown = UISwipeGestureRecognizer(target: self, action:#selector(ViewController.respondToSwipeGesture(_:)))
        swipeDown.direction = UISwipeGestureRecognizer.Direction.down //action인수는 해당 스와이프 제스처 시 실행될 메서드
        self.view.addGestureRecognizer(swipeDown)
        
        let swipeLeft = UISwipeGestureRecognizer(target: self, action:#selector(ViewController.respondToSwipeGesture(_:)))
        swipeLeft.direction = UISwipeGestureRecognizer.Direction.left//action인수는 해당 스와이프 제스처 시 실행될 메서드
        self.view.addGestureRecognizer(swipeLeft)
        
        let swipeRight = UISwipeGestureRecognizer(target: self, action:#selector(ViewController.respondToSwipeGesture(_:)))
        swipeRight.direction = UISwipeGestureRecognizer.Direction.right //action인수는 해당 스와이프 제스처 시 실행될 메서드
        ...
        swipeLeftMulti.direction = UISwipeGestureRecognizer.Direction.up
